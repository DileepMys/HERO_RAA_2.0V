...
uint8_t TX_IdleCounter[16] = {0}; // TIMEOUT AND CLEAR THE IDLE IF EXCEEDS MORE TIME
...
status_t FLEXCAN_TransferSendNonBlocking(CAN_Type *base, flexcan_handle_t *handle, flexcan_mb_transfer_t *pMbXfer)
{
    ...
    if ((uint8_t)kFLEXCAN_StateIdle == handle->mbState[pMbXfer->mbIdx])
    {
        ...
        status = FLEXCAN_WriteTxMb(base, pMbXfer->mbIdx, (const flexcan_frame_t *)(uint32_t)pMbXfer->frame);
...
/* Enable Message Buffer Interrupt. */
    ...
    else
    {
        ...
        /* COUNT THE OCCURANCE AND CHANGE THE STATE IF EXCEEDS ITS MAXIMUM TIME */
        TX_IdleCounter[pMbXfer->mbIdx]++;

        if(TX_IdleCounter[pMbXfer->mbIdx] > 5)
        {
            /* CLEAR THE COUNTER */
            TX_IdleCounter[pMbXfer->mbIdx] = 0;

            /* CHANGE THE STATE TO IDLE */
            handle->mbState[pMbXfer->mbIdx] = 0;
        }
    }
